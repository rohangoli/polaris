...
public class ServiceProducers {
  ...
  public SdkHttpClient createInsecureHttpClient(S3AccessConfig config) {
    try {
      ...
      // Create socket factory with hostname verification disabled
      SSLConnectionSocketFactory socketFactory =
          new SSLConnectionSocketFactory(sslContext, NoopHostnameVerifier.INSTANCE);

      ApacheHttpClient.Builder httpClient = ApacheHttpClient.builder().socketFactory(socketFactory);
      ...
      LOGGER.warn(
          "Creating HTTP client with SSL certificate verification disabled. "
              + "This should only be used in development environments!");

      return httpClient.build();
    } catch (Exception e) {
      LOGGER.error("Failed to create insecure HTTP client, falling back to secure client", e);
